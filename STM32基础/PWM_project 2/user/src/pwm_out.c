...
void pwm_out_init()
{
		...
		TIM3->SMCR&=~(0X7<<0);		//外设时钟源，而且其他从模式功能关闭
		TIM3->SMCR&=~(0X1<<14);//ECE禁止外部时钟模式2
		TIM3->PSC=83;//外部时钟84M分频器1HZ
		TIM3->ARR=1000-1;//定时时间为1毫秒
		TIM3->CNT=0;    //清除计数数值
		TIM3->EGR|=(0X1<<0);  //更新生成，该寄存器用于软件控制硬件事件
		TIM3->SR&=~(0X1<<0);	//清除标志位 
		TIM3->CR1=0;					//清除
...
{	
	if(TIM3->SR&(0X1<<0)) //判断定时中断标志位
	{
		...
		TIM3->CCMR1|=(0X6<<4);//控制输出的状态，当计数值比寄存器的值小时输出有效状态

		TIM3->CCER&=~(0XF<<0);

		TIM3->CCER|=(0X1<<0);//使能输出
