...
static int my_tasklet_init(void)
{
 	tasklet_init(&my_tasklet,
			my_task_func, (unsigned long)"wriken");
...
static int __init xyd_btn_init(void)
{
    ...
    unsigned long flags= IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING;
   	printk("coming\n");
    irq=gpio_to_irq(60);//0 A5 
    if(irq<0)
    {
    	printk("this is gpio error\n");
    	return 0;
    }
    if(request_irq(irq,btns_irq_handler,flags,"KEY0",NULL)<0)
    {
    	printk("this is request error\n");
    	return -0;
    }*/
    printk("gpio is ok\n");
	//计算按键数量
