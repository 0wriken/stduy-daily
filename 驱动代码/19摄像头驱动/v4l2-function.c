...
//是否编译生成yuv图片的代码，0表示不生成，1表示生成
#define HAVE_YUV_FILE  1

#define DEST_PATH     "/home/jpg"   //jpeg文件存放路径
...
int video_mainloop(void)
{
    ...
    mkdir(DEST_PATH,0666);  //创建存放图片的目录
    ...
    while (1) {
        ...
        //LCD屏上显示图片
        ret = lcd_show_rgb_image((vinfo.xres - fmt.fmt.pix.width) / 2, 0,  VID_IMG_W, VID_IMG_H, vbuffer);
        if ( ret < 0) {
        ...
        //把RGB888数据保存到jpeg图片文件中
        char file_name[125];
        static unsigned int pic_cnt = 0;
        sprintf(file_name, "%s/Photo-%05d.jpg", DEST_PATH, pic_cnt++);  //合成文件名
        //写入jpeg文件中
        ret = rgb888_write_jpeg(file_name, vbuffer, VID_IMG_W, VID_IMG_H);
        if ( ret < 0) {
            fprintf(stderr, "%s error %d, %s\n", "rgb888_write_jpeg", errno, strerror(errno));
            return ret;
        }

     
#if HAVE_YUV_FILE
        //把YUV422数据保存到 yuv 图片文件中
        sprintf(file_name, "%s/yuv422-%dx%d-%d.yuv", DEST_PATH, VID_IMG_W, VID_IMG_H, pic_cnt++); //合成文件名
        ret = yuv422_write_raw(file_name, yuv_vbuffer, buffers[index].length);
        if ( ret < 0) {
            fprintf(stderr, "%s error %d, %s\n", "yuv422_write_raw", errno, strerror(errno));
            return ret;
        }
#endif
